...
app.post('/', function (req,res) {
  ...
  if(wxType == 'METAR') {
    ...
    request(options, function(err, response, body) {
      ...
      //res.render('wx', {cityName: null, weather: null, error: null});
      res.render('wx', {cityName: city, weather: metar, error: null});
    });
  }
  else {
    ...
    request(url, function(err, response, body) {
      if(err) {
        res.render('wx', {cityName: city, error: 'Error, please try again'});
      }
      else {
        let weather = JSON.parse(body);
        if(weather.main == undefined) {
          let errorMsg = `Error: No city by the name '${city}' was found. Please check and try again!`
          res.render('wx', {cityName: null, error: errorMsg});
        }
        else {
          ...
          res.render('wx', {cityName: city, weather: currentWeather, error: null});
        }
      }
    });
  }
